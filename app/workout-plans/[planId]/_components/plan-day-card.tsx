...
export function PlanDayCard({ workoutDay, planId }: PlanDayCardProps) {
  const dayLabel = WEEK_DAY_LABELS[workoutDay.weekDay] ?? workoutDay.weekDay;

  if (workoutDay.isRest) {
    return (
      <div className="flex h-[110px] flex-col items-start justify-between rounded-xl bg-secondary p-5">
        <div className="flex items-center justify-center">
          <div className="flex items-center gap-1 rounded-full bg-foreground/8 px-2.5 py-1.5 backdrop-blur-sm">
            ...
            <span className="font-heading text-xs font-semibold uppercase leading-none text-foreground">
              {dayLabel}
            </span>
          </div>
        </div>
  ...
  return (
    <Link href={`/workout-plans/${planId}/days/${workoutDay.id}`}>
